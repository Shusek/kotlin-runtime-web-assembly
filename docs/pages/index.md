...
- `runtime`: portable WebAssembly interpreter and runtime APIs.
...
- `simd`: optional JVM-only interpreter machine for WebAssembly SIMD `v128`
  instructions, backed by the JDK incubating Vector API.
...
For JVM workloads that execute WebAssembly SIMD instructions, see the optional
`simd` artifact in the [installation guide](getting-started/installation.md)
and the [execution modes](execution/modes.md) page.

## Operational Topics

- [Security](guides/security.md): trust boundaries, host imports, and resource
  limits.
- [CPU limits](execution/cpu-limits.md): JVM interruption, timeouts, and the
  unsafe instruction listener.
- [Compiler cache](execution/compiler-cache.md): JVM bytecode cache handling.
