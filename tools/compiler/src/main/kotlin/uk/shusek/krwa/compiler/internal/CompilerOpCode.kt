...
enum class CompilerOpCode(private val opcode: OpCode? = null) {
    LABEL,
    DROP_KEEP,
    TRAP,
    GOTO,
    ...
    fun opcode(): OpCode = opcode ?: throw IllegalStateException("No opcode for $this")

    companion object {
        private val OPCODES: Map<OpCode, CompilerOpCode> =
            EnumMap<OpCode, CompilerOpCode>(OpCode::class.java).also { opcodes ->
                for (value in entries) {
                    value.opcode?.let { opcodes[it] = value }
                }
            }

        @JvmStatic
        fun of(opcode: OpCode): CompilerOpCode =
            OPCODES[opcode] ?: throw IllegalArgumentException("No compiler opcode for $opcode")
    }
}
