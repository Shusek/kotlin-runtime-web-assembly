...
- `uk.shusek.krwa:component-model` contains the multiplatform WIT model,
  canonical ABI, WASI host surfaces, and plugin runtime. It does not ship
  `wasm-tools.wasm`.
- `uk.shusek.krwa:component-model-tooling` is an optional JVM artifact for WIT
  normalization, component packaging/unbundling, validation, and
  `WasmPlugin.builderFromComponent(...)`. It installs the implementation used by
  the runtime facade through `ServiceLoader` and brings in the embedded
  `wasm-tools` executable.

The `uk.shusek.krwa.component-model` Gradle plugin includes the tooling artifact
automatically. A host that parses its WIT contract with
`WitPackage.parse(source)` and loads a core module with
...
- `Wit.normalize(...)`, `Wit.json(...)`, `Wit.encodePackage(...)`, and `Wit.parse(Path/byte[])` backed by the in-repo `wasm-tools` module running under Kotlin Runtime Web Assembly WASI Preview 1. These JVM operations require `component-model-tooling`. Component parsing reads the full WIT dependency graph, not only the root world.
...
- `WasmPlugin`, a WIT-world loader for core Wasm plugin modules and component artifacts
  that wires host imports and exported functions through `CanonicalAbi`. Core modules
  loaded with `builder(witPackage).withModule(module)` do not need tooling. Component
  artifacts are unbundled with `wasm-tools` and require `component-model-tooling`; when adapters are present, Kotlin Runtime Web Assembly selects
...
- `WasmComponentTools`, JVM helpers for `wasm-tools component embed`, `component new`, `component unbundle`, and component validation. Add `component-model-tooling` before calling them.
...
component artifact. Path-based component-model APIs use Okio `Path` and require
the JVM `component-model-tooling` artifact:
...
```shell
java -cp "<component-model-tooling runtime classpath>" uk.shusek.krwa.component.KotlinWitBindgen \
...
dependencies {
    ...
    witBindgen("uk.shusek.krwa:component-model-tooling:<version>")

    // Or, inside this repository:
    // witBindgen(project(":component-model-tooling"))
}
...
2. Use `WasmComponentTools.componentFromCore(...)` or `WasmComponentPackager`.
   This build-time step requires `component-model-tooling`.
...
4. Use `WasmPlugin.builderFromComponent(...)` to load the component. This runtime path
   also requires `component-model-tooling`. If the component
...
```shell
java -cp "<component-model-tooling runtime classpath>" uk.shusek.krwa.component.WasmComponentPackager \
  --wit wit/plugin.wit \
  --world plugin \
