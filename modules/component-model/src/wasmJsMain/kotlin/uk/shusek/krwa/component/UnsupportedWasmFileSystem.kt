...
internal class MemoryWasmFileSystem : FileSystem() {
    ...
    fun isSameFile(first: Path, second: Path): Boolean {
        val firstNode =
            entries[resolvePath(first, followFinal = true)]
                ?: throw IOException("no such file or directory: $first")
        val secondNode =
            entries[resolvePath(second, followFinal = true)]
                ?: throw IOException("no such file or directory: $second")
        return firstNode === secondNode
    }

    fun fileIdentity(path: Path): Any =
        entries[resolvePath(path, followFinal = true)]
            ?: throw IOException("no such file or directory: $path")
