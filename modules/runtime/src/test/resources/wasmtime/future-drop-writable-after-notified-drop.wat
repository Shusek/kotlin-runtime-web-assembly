;; Adapted from Wasmtime v47.0.2's regression for bytecodealliance/wasmtime#13914.
(component
  (component $A
    ...
    (core module $cm
      ...
      (func (export "write-and-wait-cb")
        (param $event i32)
        (param $waitable i32)
        (param $code i32)
        (result i32)
        (if (i32.ne (local.get $event) (i32.const 5))
          (then unreachable))
        (if (i32.ne (local.get $waitable) (global.get $writer))
          (then unreachable))
        (if (i32.ne (local.get $code) (i32.const 1))
          (then unreachable))
    ...
    (func (export "write-and-wait") async
      (canon lift
        (core func $ci "write-and-wait")
        async
        (callback (func $ci "write-and-wait-cb"))))
  )
